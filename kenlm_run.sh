...
for order in range(min_order, max_order + 1):
    ...
    for prune in range(min_order, order + 1):
        ...
        for interpolate_unigrams in range(0, 2):
            ...
            subprocess.run('/root/kenlm/build/bin/lmplz --memory 50%% --verbose_header --order %d --prune %s --interpolate_unigrams %d --text %s --arpa %s' % (order, prune_arg, interpolate_unigrams, librispeech_path, arpa_path))
            for counter, limit_vocab_file in enumerate(limit_vocab_files):
                ...
                arpa_path = os.path.join(arpa_directory, arpa_file)
                print('Generating arpa %s' % arpa_path)
                subprocess.run('/root/kenlm/build/bin/lmplz --memory 50%% --verbose_header --order %d --prune %s --limit_vocab_file %s --interpolate_unigrams %d --text %s --arpa %s' % (order, prune_arg, limit_vocab_file, interpolate_unigrams, librispeech_path, arpa_path))
...
for arpa_path in os.listdir(arpa_directory):
    if arpa_path.endswith('.arpa'):
        ...
        for space_multiplier in [x * 0.1 for x in range(11, 102, 10)]:
            ...
            subprocess.run('/root/kenlm/build/bin/build_binary -S 50%% -p %f probing %s %s' % (space_multiplier, arpa_path, probing_path) )
...
for arpa_path in os.listdir(arpa_directory):
    if arpa_path.endswith('.arpa'):
        ...
        subprocess.run('/root/kenlm/build/bin/build_binary -S 50%% trie %s %s' % (arpa_path, trie_path))
        for bits in range(1, (max_bits + 1)):
            ...
            for backoff in range(1, bits + 1):
                ...
                trie_path = os.path.join(trie_directory, trie_file)
                subprocess.run('/root/kenlm/build/bin/build_binary -S 5%0% trie -q %d -b %d %s %s' % (bits, backoff, arpa_path, trie_path))
                ...
                trie_path = os.path.join(trie_directory, trie_file)
                subprocess.run('/root/kenlm/build/bin/build_binary -S 50%% trie -q %d -b %d -a %d %s %s' % (bits, backoff, array_max_bits, arpa_path, trie_path))
